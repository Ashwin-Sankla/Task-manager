...
/**
 * Checks if the user has the SUPER_ADMIN system role.
 */
async function requireSystemAdmin(req, res, next) {
  try {
    if (req.user.systemRole !== 'SUPER_ADMIN') {
      return res.status(403).json({ error: 'System Admin access required for this action' });
    }
    next();
  } catch (err) {
    next(err);
...
module.exports = { authenticate, requireProjectAdmin, requireProjectMember, requireSystemAdmin };
